...
class _BaseConnectionAttributes:
    """
    ...
    def __init__(self, server, database, instance=INSTANCE_DEFAULT, driver=DRIVER_DEFAULT,
        ...
        if self._logger.hasHandlers():
            self._logger.debug(f"handlers already found, logging at level "
                               f"{(self._logger.getEffectiveLevel())}, continuing...")
        else:
            basicConfig(level='INFO')
            self._logger.debug(f"logger had no handlers, setup using basicConfig at level "
                               f"{self._logger.getEffectiveLevel()}, continuing...")
